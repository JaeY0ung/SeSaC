...
class AddressGenerator:
    ...
    def generate_address(self):
        ...
        self.street = random.randint(1,99)
        self.lo_gil = random.choice(['로','길'])
        self.last_num = random.randint(1,99)
        return f"{self.si} {self.gu} {self.street:02d}{self.lo_gil} {self.last_num:02d}"
...
class DataGenerator:
    def __init__(self, sung_file, irum_file, storeNames_file, si_file, gu_file, count):   # 객체 생성
        ...
        self.storetype = None
        self.data = None
        self.user_or_store = None

        while True:
            self.user_or_store = input("어떤 정보를 생성하시겠습니까?(USER / STORE): ").lower()
            if self.user_or_store == "user":
                self.initialSetting_user(si_file, gu_file)
                self.data = self.generate_userdata(sung_file, irum_file, count)
                break
                
            elif self.user_or_store == "store":
    ...
    def generate_userdata(self, sung_file, irum_file, count):
        ...
        for _ in range(count):
            ...
            keys = ['Id', 'Name', 'Birthday', 'Gender', 'Age', 'Address']
            values = [userid, username, userbirthday, usergender, userage, useraddress]
            userdata.append(dict(zip(keys,values)))
        return userdata
    ...
    def generate_storedata(self, storeNames_file, count):
        ...
        for _ in range(count):
            ...
            values = [storeid, storename, storetype, storeaddress]
            storedata.append(dict(zip(keys,values)))
        return storedata
    
class DataPrinter:
    def __init__(self):  # 객체 생성
        self.user_or_store = None
        self.console_or_csv = None

    def printData(self, data, filename):
        if self.user_or_store == "user":
            while True:
                self.console_or_csv = input("CONSOLE or CSV?: ").lower()
                if self.console_or_csv == 'console':
                    return self.userdata_export_to_console(data)
                elif self.console_or_csv == 'csv':
...
sung_file           = '/Users/jeongjaeyeong/Project/SeSac/Python_DataGenerator/Generator/src/koreanNames_sung.txt'
irum_file           = '/Users/jeongjaeyeong/Project/SeSac/Python_DataGenerator/Generator/src/koreanNames_irum.txt'
storeName_file      = '/Users/jeongjaeyeong/Project/SeSac/Python_DataGenerator/Generator/src/storeNames.txt'
city_file           = '/Users/jeongjaeyeong/Project/SeSac/Python_DataGenerator/Generator/src/koreaCities.txt'
si_file             = '/Users/jeongjaeyeong/Project/SeSac/Python_DataGenerator/Generator/src/korea_sis.txt'
gu_file             = '/Users/jeongjaeyeong/Project/SeSac/Python_DataGenerator/Generator/src/korea_gus.txt'
count_input         = int(input("몇 개의 데이터를 생성하시겠습니까?: "))

data_gen = DataGenerator(sung_file, irum_file, storeName_file, si_file, gu_file, count_input)
data = data_gen.data
export_csv_filename = f'/Users/jeongjaeyeong/Project/SeSac/Python_DataGenerator/Generator/csv/crm_{data_gen.user_or_store}.csv'

data_printer = DataPrinter()
data_printer.user_or_store =  data_gen.user_or_store
data_printer.printData(data, export_csv_filename)