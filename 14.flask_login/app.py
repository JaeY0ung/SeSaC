...
#! login_required: 데코레이터로, 특정 뷰 함수를 보호하는 데 사용됩니다. 
#!                 이 데코레이터가 적용된 뷰 함수는 로그인된 사용자만 접근할 수 있으며, 로그인되지 않은 사용자는 로그인 페이지로 리디렉션됩니다.
...
app.config["SQLALCHEMY_DATABASE_URI"] = utils.SQLALCHEMY_DATABASE_URI
...
def profile_edit():
    ...
    if request.method == "POST":
        ...
        print(f'새로운 비밀번호: {new_password}')

        #? 2. 저장할 장소(즉 현재 사용자)를 가져온다. (current_user)
...
def withdrawal():
    if request.method == 'POST':
        ...
        password = request.form['password']
        # print(current_user.username, current_user.password_hash)
